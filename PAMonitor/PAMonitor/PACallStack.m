...
typedef struct PAStackFrame {
    const struct PAStackFrame *const previous;
    const uintptr_t return_address;
} PAStackFrame;

//存储 thread 信息的结构体
typedef struct PAThreadInfoStruct {
    double cpuUsage;
    integer_t userTime;
} PAThreadInfoStruct;

static mach_port_t _paMainThreadId;

@implementation PACallStack

+ (void)load {
    _paMainThreadId = mach_thread_self();
}

#pragma mark - Interface
+ (NSString *)callStackWithType:(PACallStackType)type {
    if (type == PACallStackTypeAll) {
        ...
        for (int i = 0; i < thread_count; i++) {
            //当前执行的指令
            [reStr appendString:paStackOfThread(threads[i])];
        }
        //task info
        NSString *memStr = @"";
        ...
        return [reStr copy];
    } else if (type == PACallStackTypeMain) {
        //主线程
        NSString *reStr = paStackOfThread((thread_t)_paMainThreadId);
        assert(vm_deallocate(mach_task_self(), (vm_address_t)_paMainThreadId, 1 * sizeof(thread_t)) == KERN_SUCCESS);
        PALog(@"%@",reStr);
        return [reStr copy];
    } else {
        //当前线程
        ...
        [nsthread setName:originName];
        reStr = paStackOfThread(mach_thread_self());
        PALog(@"%@",reStr);
        return [reStr copy];
    }
    return @"";
...
//当前栈帧结构
NSString *paStackOfThread(thread_t thread) {
    //thread info
    PAThreadInfoStruct threadInfoSt = {0};
    ...
    mach_msg_type_number_t state_count = paThreadStateCountByCPU();
    kern_return_t kr = thread_get_state(thread, paThreadStateByCPU(), (thread_state_t)&machineContext.__ss, &state_count);
    if (kr != KERN_SUCCESS) {
        return [NSString stringWithFormat:@"Fail get thread: %u", thread];
    }
    //通过指令指针来获取当前指令地址
    const uintptr_t instructionAddress = paMachInstructionPointerByCPU(&machineContext);
    buffer[i] = instructionAddress;
    ++i;
    
    //
    uintptr_t linkRegisterPointer = paMachThreadGetLinkRegisterPointerByCPU(&machineContext);
    ...
    PAStackFrame stackFrame = {0};
    //通过栈基址指针获取当前栈帧地址
    const uintptr_t framePointer = paMachStackBasePointerByCPU(&machineContext);
    if (framePointer == 0 || paMemCopySafely((void *)framePointer, &stackFrame, sizeof(stackFrame)) != KERN_SUCCESS) {
        return @"Fail frame pointer";
    }
    for (; i < 32; i++) {
        buffer[i] = stackFrame.return_address;
        if (buffer[i] == 0 || stackFrame.previous == 0 || paMemCopySafely(stackFrame.previous, &stackFrame, sizeof(stackFrame)) != KERN_SUCCESS) {
            break;
        }
    }
    ...
    paSymbolicate(buffer, symbolicated, stackLength, 0);
    for (int i = 0; i < stackLength; ++i) {
        [reStr appendFormat:@"%@", paOutputLog(i, buffer[i], &symbolicated[i])];
    }
    [reStr appendFormat:@"\n"];
    return reStr;
}
...
NSString* paOutputLog(const int entryNum, const uintptr_t address, const Dl_info* const dlInfo) {
...
kern_return_t paMemCopySafely(const void *const src, void *const dst, const size_t byteSize) {
    vm_size_t bytesCopied = 0;
    return vm_read_overwrite(mach_task_self(), (vm_address_t)src, (vm_size_t)byteSize, (vm_address_t)dst, &bytesCopied);
}

#pragma mark - Symbolicate
void paSymbolicate(const uintptr_t* const stackBuffer, Dl_info* const symbolsBuffer, const int stackLength, const int skippedEntries) {
    int i = 0;
    if (!skippedEntries && i < stackLength) {
        paDladdr(stackBuffer[i], &symbolsBuffer[i]);
        i++;
    }
    for (; i < stackLength; i++) {
        paDladdr(paInstructionAddressByCPU(stackBuffer[i]), &symbolsBuffer[i]);
    }
}
bool paDladdr(const uintptr_t address, Dl_info* const info) {
    info->dli_fname = NULL;
    info->dli_fbase = NULL;
    info->dli_sname = NULL;
    info->dli_saddr = NULL;
    //根据地址获取是哪个 image
    const uint32_t idx = paDyldImageIndexFromAddress(address);
    if (idx == UINT_MAX) {
        return false;
    }
    /*
    ...
    const uintptr_t segmentBase = paSegmentBaseOfImageIndex(idx) + imageVMAddressSlide;
    if (segmentBase == 0) {
        return false;
    }
    //
    ...
    uintptr_t cmdPointer = paCmdFirstPointerFromMachHeader(machHeader);
    if (cmdPointer == 0) {
        return false;
    }
...
uint32_t paDyldImageIndexFromAddress(const uintptr_t address) {
...
uintptr_t paCmdFirstPointerFromMachHeader(const struct mach_header* const machHeader) {
...
uintptr_t paSegmentBaseOfImageIndex(const uint32_t idx) {
    const struct mach_header* machHeader = _dyld_get_image_header(idx);
    //查找 segment command 返回 image 的地址
    uintptr_t cmdPtr = paCmdFirstPointerFromMachHeader(machHeader);
    if (cmdPtr == 0) {
        return 0;
    }
 ...
 */
uintptr_t paMachStackBasePointerByCPU(mcontext_t const machineContext) {
...
uintptr_t paMachInstructionPointerByCPU(mcontext_t const machineContext) {
...
uintptr_t paInstructionAddressByCPU(const uintptr_t address) {
#if defined(__arm64__)
...
mach_msg_type_number_t paThreadStateCountByCPU() {
#if defined(__arm64__)
 ...
 */
thread_state_flavor_t paThreadStateByCPU() {
#if defined(__arm64__)
...
uintptr_t paMachThreadGetLinkRegisterPointerByCPU(mcontext_t const machineContext) {
#if defined(__i386__)
    return 0;
#elif defined(__x86_64__)
    return 0;
