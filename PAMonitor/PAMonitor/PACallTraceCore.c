...
static paCallRecord *_smCallRecords;
...
static inline uintptr_t pop_call_record() {
    ...
    if (cs->is_main_thread && _call_record_enabled) {
        ...
        if (time < pRecord->time) {//如果出现第二次取模小于第一次，只能是第二次模了100后，比如之前是96，后来变成120等
            time += 100 * 1000000;
        }
        ...
        if (cost > _min_time_cost && cs->index < _max_call_depth) {
            if (!_smCallRecords) {
                _smRecordAlloc = 1024;
                _smCallRecords = malloc(sizeof(paCallRecord) * _smRecordAlloc);
            }
            _smRecordNum++;
            if (_smRecordNum >= _smRecordAlloc) {
                _smRecordAlloc += 1024;
                _smCallRecords = realloc(_smCallRecords, sizeof(paCallRecord) * _smRecordAlloc);
            }
            paCallRecord *log = &_smCallRecords[_smRecordNum - 1];
            log->cls = pRecord->cls;
            log->depth = curIndex;
            log->sel = pRecord->cmd;
            log->time = cost;
...
void paCallTraceStop(void) {
    _call_record_enabled = false;
}

void paCallConfigMinTime(uint64_t us) {
    _min_time_cost = us;
}
void paCallConfigMaxDepth(int depth) {
    _max_call_depth = depth;
}

paCallRecord *paGetCallRecords(int *num) {
    if (num) {
        *num = _smRecordNum;
    }
