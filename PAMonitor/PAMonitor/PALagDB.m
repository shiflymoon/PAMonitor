...
@implementation PALagDB

+(instancetype) sharedInstance
{
    static PALagDB * _instance = nil;
    ...
    if (self = [super init]) {
        ...
        if ([[NSFileManager defaultManager] fileExistsAtPath:_clsCallDBPath] == NO) {
            ...
            if ([db open]) {
                 ...
                 */
                NSString *createSql = @"create table if not exists clscall (cid INTEGER PRIMARY KEY AUTOINCREMENT  NOT NULL, fid integer, cls text, mtd text, path text, timecost double, calldepth integer, frequency integer, lastcall integer)";
                ...
                /* stack 表记录
                 ...
                 insertdate: 日期
                 */
                NSString *createStackSql = @"create table if not exists stack (sid INTEGER PRIMARY KEY AUTOINCREMENT  NOT NULL, stackcontent text,isstuck integer, insertdate double)";
...
- (NSArray<PACallTraceTimeCostModel *> *)selectClsCallWithPage:(NSUInteger)page {
    ...
    if ([db open]) {
        ...
        NSMutableArray *arr = [NSMutableArray array];
        while ([rs next]) {
            PACallTraceTimeCostModel *model = [self clsCallModelFromResultSet:rs];
            [arr addObject:model];
        }
        [db close];
        return arr;
...
//结果封装成 model
- (PACallTraceTimeCostModel *)clsCallModelFromResultSet:(FMResultSet *)rs {
    PACallTraceTimeCostModel *model = [[PACallTraceTimeCostModel alloc] init];
